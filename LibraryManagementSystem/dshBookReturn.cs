...
namespace LibraryManagementSystem
{
    public partial class dshBookReturn : Form
    {
        ...
        private void BookReturn_Load(object sender, EventArgs e)
        {
            ...
            // TODO: This line of code loads data into the 'lmsdcsDataSet37.ActiveBorrowers' table. You can move, or remove it, as needed.
            this.activeBorrowersTableAdapter.Fill(this.lmsdcsDataSet37.ActiveBorrowers);
        ...
        /*private void SearchBookBorrowing(string searchText)
        {
            ...
            // SQL query to include BookTitle
            string query = @"
    SELECT 
        bb.BorrowID,
        bb.BorrowerID,
    ...
    WHERE ";
            ...
            try
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        // Check if BookTitle column exists, and safely update its header text
            ...
            {
                MessageBox.Show("A null reference occurred: " + nre.Message);
            }
...
ORDER BY 
    ...
    END";
        ...
        private void overDueCheck_Tick(object sender, EventArgs e)
        {
            ...
            // Query to find overdue books that haven't been notified
            string query = "SELECT bb.BorrowerID, bb.SerialNumber, bb.BookTitle, bb.DueDate " +
                           "FROM BookBorrowing bb " +
                           "WHERE bb.DueDate < @currentDate AND bb.Status = 'Borrowed' AND bb.OverdueNotified = 0";

            using (SqlConnection conn = new SqlConnection(connectionString))
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                // Add current date parameter to the query
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        // Check the type of SerialNumber
                        string serialNumber = reader[1].ToString(); // Convert to string dynamically
                        string bookTitle = reader.GetString(2);
                        DateTime dueDate = reader.GetDateTime(3);

                        // Show notification for overdue book
                        ShowOverdueNotification(borrowerID, bookTitle, dueDate);

                        // Mark the book as notified
